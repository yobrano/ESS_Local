...
const MDNewTrainingNeedsUpdateCard = (props) => {
  ...
  const [genesisPoint, setGenesisPoint] = useState("");
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/training/gettrainingneedlist/${props.location.state[0].datum[0].needNo}`,
        config
      )
      ...
      .then(function (response) {
        if (response.status === 200) {
          ...
          if (Object.keys(response.data.clender).length > 0) {
            setSelectedCalenderYear(response.data.clender);
          }
          getCalDate();
          setEmployeeList(response.data.employeeList);
          //Disable/Enable push to FMD if-ifnot on stage TWO
          if (props.location.state[0].datum[0].stage != 3) {
            setDisablePushToHR(true); //disabled
          }
          //Disable/Enable push to HOD if genesis point is =/!= current stage
          



          //setup the comments from DB
          setHRComment(response.data.comments.hrComment)
          setMFDComment(response.data.comments.fmdComment)
          setHODComment(response.data.comments.hodComment)
          setGenesisPoint(response.data.comments.genesisPoint)
  ...
  const pushToHRa = (e)=>{
    ...
    swal({
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.post(
            `${process.env.REACT_APP_API_S_LINK}/training/pushtrainingneedfrommfdtohr/`,
            Data,
            config
          );
        }
  ...
  const pushToHODa = (e)=>{
    ...
    swal({
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.post(
            `${process.env.REACT_APP_API_S_LINK}/training/pushtrainingneedfrommfdtohod/`,
            Data,
            config
          );
        }
  ...
  const pushToApprovea = (e)=>{
    e.preventDefault();
   
    if (mfdComment == "" || mfdComment == null) {
      swal("Oh!","MFD Comment is Empty", "error");
      return
    }
    ...
    swal({
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.post(
            `${process.env.REACT_APP_API_S_LINK}/training/pushtrainingneedfrommdtoapprove/`,
            Data,
            config
          );
        }
  ...
  if(genesisPoint == "3"){
    //Approve- Final Step
    buttonActions =(
      <button  className="btn btn-warning mt-3" onClick={pushToHODa}>
       {" "}
  ...
  // }
    ...
    //other Options
    buttonActions=(
    //   <button className="btn btn-warning mt-3" onClick={pushToHRa} disabled={disablePushToHR}>
              
    //   {" "}
    //   Push to HR <i className="fa fa-arrow-right"></i>
    // </button>
    <button  className="btn btn-info mt-3" onClick={pushToApprovea}>
       <i className="	fa fa-hourglass-end"></i> Approve
    </button>
    )
  }
      ...
      <div className="container">
        ...
        <div className="card">
          ...
          <div className="card-footer">
            ...
            <div className="col-md-12">
                <div className="form-group">
                  <label htmlFor="">MD Comment</label>
                  <textarea
                    className="form-control"
                    id=""
                    rows="1"
                    ...
                    name="mfdComment"
                    value={mfdComment}
                    onChange={(e) => setMFDComment(e.target.value)}
                  ></textarea>
...
export default withRouter(MDNewTrainingNeedsUpdateCard);
