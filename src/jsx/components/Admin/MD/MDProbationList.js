...
function MDProbationList(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/endofmonitoringandcontract/getfdprobationlist`,
        config
      )
      .then((result) => {
  ...
  if (selection.length === 1) {
    ...
    if(selection[0].probationNo !== undefined){
      props.history.push("/MD-probation-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(MDProbationList);
