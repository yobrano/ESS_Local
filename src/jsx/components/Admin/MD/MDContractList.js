...
function MDContractList(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/endofmonitoringandcontract/getfdcontractlist`,
        config
      )
      .then((result) => {
  ...
  if (selection.length === 1) {
    ...
    if(selection[0].contractNo !== undefined){
      props.history.push("/MD-contract-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(MDContractList);
