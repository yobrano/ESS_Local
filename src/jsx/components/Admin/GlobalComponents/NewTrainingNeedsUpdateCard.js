...
const NewTrainingNeedsUpdateCard = (props) => {
  ...
  const [disablePushToHOD,setDisablePushToHOD] = useState(false)
    ...
    axios
      ...
      .then(function (response) {
        if (response.status === 200) {
          ...
          //Disable/Enable push to HOD if-ifnot on stage zero
          if(props.location.state[0].datum[0].stage!=0){
            setDisablePushToHOD(true)//disabled
          }
          setLoading(false);
        }
  ...
  const pushToHODa = (e)=>{
    ...
    swal({
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.get(
            `${process.env.REACT_APP_API_S_LINK}/training/pushtrainingneedfromstafftohod/${trainingNeedNo}/`,
            // Data,
            config
          );
        }
    ...
    <>
      <div className="container">
        ...
        <div className="card">
          ...
          <div className="card-footer"> 
            <button className="btn btn-warning" onClick={pushToHODa} disabled={disablePushToHOD}> Push to HOD <i className="fa fa-arrow-right"></i></button>
          </div>
        </div>
      </div>
    </>
...
export default withRouter(NewTrainingNeedsUpdateCard);
