...
function CompetenceListApprove(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/competence/getsupercompetencelist`,
        config
      )
      .then((result) => {
        setData(result.data.competenceFrameworks);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  if (selection.length === 1) {
    if (selection[0].cno !== undefined) {
      props.history.push("/approve-competency-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(CompetenceListApprove);
