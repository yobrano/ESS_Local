...
function SupervisorAppraisalList(props) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_S_LINK}/performanceevaluation/getsupervisorappraisals`, config)
      .then((result) => {
        console.log(result.data);
        setData(result.data.supervisorAppraisalLists);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  if (selection.length === 1) {
    ...
    if (selection[0].no !== undefined) {
      ...
      //   }}
      // />;
      props.history.push("/view-supervisor-appraisal", [{ datum: selection }]);
    }
  }
...
export default withRouter(SupervisorAppraisalList);
