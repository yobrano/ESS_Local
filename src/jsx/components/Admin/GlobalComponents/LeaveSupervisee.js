...
const DataTable = ({ columns, data, setSelection, props }) => {
  ...
  const createNewLeave = () => {
    props.history.push("/new-leave", []);
  };

  // Render the UI for your table
  const xcelBtn =
    excelBtn === true ? (
      <button className="btn btn-success rounded-0" onClick={createNewLeave}>
        Create a New Leave <i className="flaticon-381-calendar-2 ms-3 "></i>
      </button>
    ) : (
      <button className="btn btn-success d-none">Create a New Leave</button>
    );
...
function LeaveSupervisee(props) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_S_LINK}/leave/getemployeepermanager`, config)
      .then((result) => {
        // =>console.log(result.data);
        setData(result.data.managerEmployees);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  const columns = useMemo(
    () => [
      ...
      // {
      //   Header: `Product`,
      //   accessor: "globalDimension1Code",
      // },

      // {
      //   Header: `Branch`,
      //   accessor: "globalDimension2Code",
      // },
      // {
      //   Header: `Department`,
      //   accessor: "shortcutDimension3Code",
      // },
  ...
  if (selection.length === 1) {
    // =>console.log(selection[0].employeeNo);
    if (selection[0].employeeNo !== undefined) {
      props.history.push("/supervisees-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(LeaveSupervisee);
