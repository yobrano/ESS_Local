...
function EmployeeCompleteAppraisalList(props) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_S_LINK}/performanceevaluation/getsupervisorcompleted`, config)
      .then((result) => {
  ...
  if (selection.length === 1) {
    ...
    if (selection[0].no !== undefined) {
      ...
      //   }}
      // />;
      props.history.push("/view-completed", [{ datum: selection }]);
    }
  }
...
export default withRouter(EmployeeCompleteAppraisalList);
