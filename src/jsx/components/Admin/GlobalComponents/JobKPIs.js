...
function JobKPIs(props) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_S_LINK}/performance/getjobkpis`, config)
      .then((result) => {
        // =>console.log(result.data);
        setData(result.data.jobKPIs);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  if (selection.length === 1) {
    ...
    if (selection[0].no !== undefined) {
      ...
      //   }}
      // />;
      props.history.push("/KPI-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(JobKPIs);
