...
function GrievanceAppealList(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/grievance/getcthreesfivegrievancelist`,
        config
      )
      .then((result) => {
  ...
  if (selection.length === 1) {
    ...
    if (selection[0].gid !== undefined) {
      props.history.push("/grievance-appeal-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(GrievanceAppealList);
