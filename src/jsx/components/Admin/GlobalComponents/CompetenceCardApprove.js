...
const CompetenceCardApprove = (props) => {
  ...
  //Calc Score
  const supervisorCalcScore = (e) => {
    e.preventDefault();

    if (supervisorRecommendation === "") {
      let error = false;
      const errorObj = { ...errorsObj };
      errorObj.supRecomm = "Comment Required";
      error = true;
      handleClickScroll();
    ...
    let udata={
      Cno:props.location.state[0].datum[0].cno,
      GeneralSupervisorcomment:supervisorRecommendation
    }
    swal({
      title: "Are you sure?",
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.post(
            `${process.env.REACT_APP_API_S_LINK}/competence/supervisorcalculatescore/`,
            udata,
            config
          );
        }
  ...
  const handleClickScroll = () => {
    const element = document.getElementById("supCommentCard");
    if (element) {
    ...
    <>
      <div className="d-md-flex">
        ...
        <button
          ...
          onClick={supervisorCalcScore}
          disabled={!getReportDisabled}
        >
          Calculate Score and Approve
        </button>
        <button
          className="btn btn-info ml-md-1"
          onClick={GetReport}
          disabled={getReportDisabled}
        >
          Get Report
      ...
      <div className="card mt-3" id="supCommentCard">
          <div className="card-header">
          ...
          <div className="row">
            <div className="col-md-12">
              <div className="form-group">
                ...
                ></textarea>
                {errors.supRecomm && (
                  <div className="text-danger fs-12">{errors.supRecomm}</div>
                )}
              </div>
            </div>
          </div>
