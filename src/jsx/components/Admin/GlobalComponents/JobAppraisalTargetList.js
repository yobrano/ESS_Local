...
function JobAppraisalTargetList(props) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_S_LINK}/performance/getemployeetargets`, config)
      .then((result) => {
        console.log(result.data);
        setData(result.data.employeeAppraisalList);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  if (selection.length === 1) {
    ...
    if (selection[0].no !== undefined) {
      ...
      //   }}
      // />;
      props.history.push("/appraisal-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(JobAppraisalTargetList);
