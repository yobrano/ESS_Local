...
const SupervisorAppraisalCard = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/performance/getsupervisorsinglekpi/${props.location.state[0].datum[0].no}`,
        config
      )
  ...
  useEffect(() => {
    if (selectedKPI.value !== undefined) {
      ...
      axios
        .get(
          `${process.env.REACT_APP_API_S_LINK}/performanceevaluation/getsupervisorappraisalperformancestdperkpi/${selectedKPI.value}/${props.location.state[0].datum[0].no}`,
          config
        )
        .then(function (response) {
          if (response.status === 200) {
  ...
  const InitialStdLoad = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/performanceevaluation/getsupervisorappraisalperformancestd/${props.location.state[0].datum[0].no}`,
        config
      )
      .then(function (response) {
        if (response.status === 200) {
  ...
  const CalculateWeight = () => {
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          return axios.get(
            `${process.env.REACT_APP_API_S_LINK}/performanceevaluation/supervisorcalculateweight/${props.location.state[0].datum[0].no}`,
            config
          );
        }
      })
    ...
    const SubmitSuperviser = () => {
      ...
      swal({
        ...
        .then((willDelete) => {
          if (willDelete) {
            return axios.get(
              `${process.env.REACT_APP_API_S_LINK}/performanceevaluation/superviserapprove/${props.location.state[0].datum[0].no}`,
              config
            );
          }
        })
  ...
  return (
    <>
      ...
      <BreadCrumb props={props} backlink={"supervisor-appraisal"}/>
        {/* <div className="row">
                ...
                <tbody>
                  {jobkpiList.map((d, i) => (
                    <tr key={i}>
                      ...
                      <td>{d.weightedScoreSupervisor}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
