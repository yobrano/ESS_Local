...
function SupervisorModeratedAppraisalList(props) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_S_LINK}/performanceevaluation/getsupervisormoderated`, config)
      .then((result) => {
        console.log(result.data);
        setData(result.data.supervisorAppraisalModeratedLists);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "appraisalPeriod",
      },
      {
        Header: "Start Period",
        accessor: "appraisalStartPeriod",
      },
      {
        Header: "End Period",
        accessor: "appraisalEndPeriod",
      },
      // {
      //   Header: "Appraisal Stage",
      //   accessor: "appraisalLevel",
  ...
  if (selection.length === 1) {
    ...
    if (selection[0].no !== undefined) {
      ...
      //   to={{
      //     pathname: "/KPICard",
      //     state: { datum: selection },
      //   }}
      // />;
      props.history.push("view-supervisor-moderation", [{ datum: selection }]);
    }
  }
...
export default withRouter(SupervisorModeratedAppraisalList);
