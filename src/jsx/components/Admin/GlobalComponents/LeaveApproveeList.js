...
function LeaveApproveeList(props) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_S_LINK}/leave/getapprovee`, config)
      .then((result) => {
        console.log(result.data);
        setData(result.data.approvees);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  const columns = useMemo(
    () => [
      ...
      {
        Header: `Date & Time`,
        accessor: "dateTimeSentforApproval",
      },

      {
        Header: `Sender ID`,
        accessor: "senderEmployeeNo",
      },
      // {
      //   Header: `Name`,
      //   accessor: "senderEmployeeName",
      // },
  ...
  if (selection.length === 1) {
    console.log(selection[0].documentNo);
    if (selection[0].documentNo !== undefined) {
      props.history.push("/approval-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(LeaveApproveeList);
