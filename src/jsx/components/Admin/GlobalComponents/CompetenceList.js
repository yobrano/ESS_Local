...
function CompetenceList(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/competence/getstaffcompetencelist`,
        config
      )
      .then((result) => {
  ...
  if (selection.length === 1) {
    if (selection[0].cno !== undefined) {
      props.history.push("/competency-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(CompetenceList);
