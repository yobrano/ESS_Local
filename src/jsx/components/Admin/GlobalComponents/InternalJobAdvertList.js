...
function InternalJobAdvertList(props) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_S_LINK}/home/internal-vacancy`, config)
      .then((result) => {
        console.log(result.data);
        setData(result.data.postedJobsList);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  if (selection.length === 1) {
    ...
    if (selection[0].no !== undefined) {
      ...
      //   }}
      // />;
      props.history.push("/vacancy-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(InternalJobAdvertList);
