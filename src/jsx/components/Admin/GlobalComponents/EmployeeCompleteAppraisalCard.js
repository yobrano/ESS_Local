...
const EmployeeCompleteAppraisalCard = (props) => {
    ...
    axios
      ...
      .then(function (response) {
        if (response.status === 200) {
          ...
          setAreaofDevelopmentList1(response.data.areaofDevelopmentList1)
  ...
  const ViewCompleteReport = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/performanceevaluation/viewreport/${props.location.state[0].datum[0].no}`,
        config
      )
    ...
    <>
      ...
      <BreadCrumb props={props} backlink={""}/>
        <div className="d-md-flex justify-content-end mb-2">
          ...
          <button className="btn btn-info rounded-0" onClick={ViewCompleteReport} >
            Complete Appraisal Report <i className="fa fa-file-pdf-o ml-2"></i>
          </button>
        </div>
        ...
        <div className="card my-4 d-none">
...
export default withRouter(EmployeeCompleteAppraisalCard);
