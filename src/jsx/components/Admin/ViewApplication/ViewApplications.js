...
const DataTable = ({ columns, data, setSelection }) => {
  ...
  const rejectPendingApplicants = () => {
    ...
    swal({
      title: "Are you sure?",
      text: "Are you sure that you want to Reject Pending Application",
      icon: "warning",
      dangerMode: true,
    })
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.get(
            `${process.env.REACT_APP_API_S_LINK}/home/rejectpendingapplicant/`,
            config
          );
        }
      })
  ...
  const xcelBtn =
    excelBtn === true ? (
      <button className="btn btn-success ml-3" onClick={getXcel}>
        Get Excel
      </button>
    ) : (
    ...
    <>
      <div className="row">
        ...
        <div className="col-md-6 text-right">
          <button className="btn btn-danger " onClick={rejectPendingApplicants}>
            Dismiss Applicants
          </button>
          {xcelBtn}
        </div>
      ...
      },
      {
        ...
        Cell: ({ value }) => {
          return (
            ...
            >
              <span className="action-btn">{"Pending View"}</span>
            </button>
          );
        },
      },
