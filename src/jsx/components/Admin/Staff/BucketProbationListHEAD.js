...
function BucketProbationListHEAD(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/endofmonitoringandcontract/getheadprobationbucketlist`,
        config
      )
      .then((result) => {
        console.log(result.data);
        setData(result.data.employeeProbations);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  if (selection.length === 1) {
    ...
    if(selection[0].probationNo !== undefined){
      props.history.push("/bucket-probation-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(BucketProbationListHEAD);
