...
function HODADMINClearanceList(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/home/hodadmingetrespectivelist`,
        config
      )
      .then((result) => {
  ...
  const columns = useMemo(
    () => [
      ...
      //         <span class="action-btn">Pending HR Apprval/span>
      //       </button>
      //     `;
      //     }
  ...
  if (selection.length === 1) {
    ...
    props.history.push("/ADMIN-clearance-card", [{ datum: selection }]);
  }
...
export default withRouter(HODADMINClearanceList);
