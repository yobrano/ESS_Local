...
const EmployeeRequision = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/staffrequision/jobslist`,
        config
      )
      .then(function (response) {
        if (response.status === 200) {
          // =>console.log(response.data);
          setJobList(response.data.jobLists);
          populateReqList();
          setLoading(false);
        }
  ...
  const viewmoreOfReqDataRequirement = () => {
    ...
    swal({
      ...
      .then((willProcede) => {
        if (willProcede) {
          return axios.get(
            `${process.env.REACT_APP_API_S_LINK}/staffrequision/getempreqcode/${selectedJob.value}`,
            config
          );
        }
      })
      .then(function (response) {
        if (response.status === 200) {
          return props.history.push("/employee-requisition-card", [
            { empReqNo: response.data, jobNo: selectedJob },
          ]);
        }
        if (response.status === 404) {
          swal("Oh!", response.data.message, "error");
  ...
  const driftButton =
    selectedJob !== "" ? (
      <button
        ...
        onClick={viewmoreOfReqDataRequirement}
      >
