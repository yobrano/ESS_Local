...
const NewTrainingNeedsUpdateHODCard = (props) => {
    ...
    axios
      ...
      .then(function (response) {
        if (response.status === 200) {
          ...
          //Disable/Enable push to HOD if-ifnot on stage one
          if (props.location.state[0].datum[0].stage == 1) {
            setDisablePushToHR(false); //enabled
          }
  ...
  const pushToHRa = (e) => {
    ...
    swal({
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.post(
            `${process.env.REACT_APP_API_S_LINK}/training/pushtrainingneedfromhodtohr/`,
            Data,
            config
          );
        }
  ...
  const pushToApprovea = (e) => {
    ...
    if (hodComment == "" || hodComment == null) {
      swal("Oh!", "HOD Comment is Empty", "error");
      return;
    }
    ...
    swal({
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.post(
            `${process.env.REACT_APP_API_S_LINK}/training/pushtrainingneedfromhodtoapprove/`,
            Data,
            config
          );
        }
  ...
  const pushToMFDa = (e) => {
    ...
    swal({
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.post(
            `${process.env.REACT_APP_API_S_LINK}/training/pushtrainingneedfromhodtomdf/`,
            Data,
            config
          );
        }
  ...
  //Genesis is HR
  if (genesisPoint == "2") {
    //Approve- Push to M/FD Step
    //   <i className="	fa fa-hourglass-end"></i>
    buttonActions = (
      <button className="btn btn-warning mt-3" onClick={pushToMFDa}>
...
export default withRouter(NewTrainingNeedsUpdateHODCard);
