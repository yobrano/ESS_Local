...
const PerformanceList = (props) => {
    ...
    const config = {
      ...
      useEffect(() => {
        ...
        axios
          .get(
            `${process.env.REACT_APP_API_S_LINK}/staffrequision/monitoringlist`,
            config
          )
          .then((result) => {
  ...
  if (selection.length === 1) {
    props.history.push("/monitoring-card", [{ datum: selection }]);
  }
    return (
        <>
         {/* List of Headers */}
...
export default withRouter(PerformanceList);