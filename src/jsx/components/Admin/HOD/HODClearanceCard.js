...
const HODClearanceCard = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/home/hodgetcardclearancedata/${props.location.state[0].datum[0].clearanceNo}`,
        config
      )
    ...
    //   };
      ...
      swal({
        ...
        .then((willCreate) => {
          if (willCreate) {
            return axios.get(
              `${process.env.REACT_APP_API_S_LINK}/home/hodupdateclearancerecord/${props.location.state[0].datum[0].id}`,
            //   data,
              config
            );
          }
  ...
  let zeroPro = (
    <button className="btn btn-warning rounded-0 w-100" onClick={pushToHODAMIN}>
      Approve &amp; Push To HR <i className="fa fa-user"></i>
    </button>
  );
  let onePro = (
 ...
 if(props.location.state[0].datum[0].hodApproved==="TRUE"){
...
export default withRouter(HODClearanceCard);
