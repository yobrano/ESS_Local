...
function RequisitionApprovalHead(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/staffrequision/headhrgetrequisitionlist`,
        config
      )
      .then((result) => {
  ...
  if (selection.length === 1) {
    ...
    props.history.push("/HR-requisition-card", [{ datum: selection }]);
  }
...
export default withRouter(RequisitionApprovalHead);
