...
const HRNewTrainingNeedsUpdateCard = (props) => {
    ...
    axios
      ...
      .then(function (response) {
        if (response.status === 200) {
          ...
          if (props.location.state[0].datum[0].stage != 2) {
  ...
  const pushToMDFa = (e)=>{
    ...
    if (hrComment == "" || hrComment == null) {
      swal("Oh!","HR Comment is Empty", "error");
      return
    }
    let Data = {
    ...
    swal({
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.post(
            `${process.env.REACT_APP_API_S_LINK}/training/pushtrainingneedfromhrtomfd/`,
            Data,
            config
          );
        }
  ...
  const pushToApprovea = (e)=>{
    ...
    swal({
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.post(
            `${process.env.REACT_APP_API_S_LINK}/training/pushtrainingneedfromhrtoapprove/`,
            Data,
            config
          );
        }
  ...
  if(genesisPoint == "3"){
    ...
    //other Options
    // buttonActions=(
    //   <button className="btn btn-warning mt-3" onClick={pushToMDFa} disabled={disablePushToHR}>
              
    //   {" "}
    //   Push to MD/FD <i className="fa fa-arrow-right"></i>
    // </button>
    buttonActions=(
      <button className="btn btn-warning mt-3" onClick={pushToMDFa} disabled={disablePushToHR}>
...
export default withRouter(HRNewTrainingNeedsUpdateCard);
