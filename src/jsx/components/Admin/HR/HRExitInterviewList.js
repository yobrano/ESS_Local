...
function HRExitInterviewList(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/home/getpushedexitinterview`,
        config
      )
      .then((result) => {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        Header: "Separation Grounds",
        accessor: "separationGround",
  ...
  if (selection.length === 1) {
    ...
    props.history.push("/HR-exit-form", [{ datum: selection }]);
  }
...
export default withRouter(HRExitInterviewList);
