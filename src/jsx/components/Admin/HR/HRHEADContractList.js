...
function HRHEADContractList(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/endofmonitoringandcontract/getheadhrcontractlist`,
        config
      )
      .then((result) => {
  ...
  if (selection.length === 1) {
    ...
    if(selection[0].contractNo !== undefined){
      props.history.push("/HR-contract-card", [{ datum: selection }]);
    }
  }
...
export default withRouter(HRHEADContractList);
