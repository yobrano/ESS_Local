...
const DataTable = ({ columns, data, setSelection }) => {
  ...
  const getXcel = () => {
    ...
    swal({
      ...
      .then((willCreate) => {
        ...
        if (willCreate) {
          return axios.get(
            `${process.env.REACT_APP_API_S_LINK}/home/hrdeletealljobseekers`,
            config
          );
        }
      })
  ...
  const xcelBtn =(
      <button className="btn btn-danger " onClick={getXcel}>
...
function HRJobseekerList(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_S_LINK}/home/hralljobseekers`,
        config
      )
      .then((result) => {
...
export default withRouter(HRJobseekerList);
