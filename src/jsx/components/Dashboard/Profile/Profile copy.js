...
function Profile() {
  ...
  const [country, setCountry] = useState({ label: "", value: "" }); //tble req
  const [county, setCounty] = useState({ label: "", value: "" }); //tble req
  const [subcounty, setSubcounty] = useState({ label: "", value: "" }); //tble req
  ...
  const [subcountyList, subsetCountyList] = useState([]);
    ...
    axios
      ...
      .then(function (response) {
        if (response.status === 200) {
          ...
          setCountryList(response.data.countryModels);
          setCountyList(response.data.countyModels);
  ...
  useEffect(() => {
    if (county.value !== "") {
      ...
      axios
        .get(
          `${process.env.REACT_APP_API_S_LINK}/profile/getsubcounty/${county.value}`,
          config
        )
        .then(function (response) {
          if (response.status === 200) {
  ...
  const changeProfile = (e) => {
    ...
    if (surName === "") {
      errorObj.surName = "surName is Required";
      error = true;
    }
    if (firstName === "") {
    ...
    if (disabled === "") {
      errorObj.disabled = "Disability check is Required";
      error = true;
    }
    if (highestAcademicLevel === "") {
      errorObj.highestAcademicLevel = "Academic level required";
      error = true;
    }
    if (willingToRelocate === "") {
      errorObj.willingToRelocate = "Willing To Relocate required";
      error = true;
    }
    if (startDate === "") {
      errorObj.startDate = "DOB required";
      error = true;
    }
    if (age === "") {
      errorObj.age = "Age required";
      error = true;
    }
    setErrors(errorObj);
    if (error) {
